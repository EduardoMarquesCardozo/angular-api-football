...
describe('StatisticComponent', () => {
  ...
  it('should sort lineup by number of games played', (done: DoneFn) => {
    if (component.lineup$) {
      component.lineup$.subscribe((lineup) => {
        for (let i = 0; i < lineup.length - 1; i++) {
          expect(lineup[i].played).toBeGreaterThanOrEqual(lineup[i + 1].played);
        }
        done();
      });
    } else {
      fail('lineup$ is undefined');
    }
  });
});
